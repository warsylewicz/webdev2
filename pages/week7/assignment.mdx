...
1. Use another student's Week 6 Assignment as a starting point for this week's assignment. Refer to the list of [student repositories](/github-accounts).

2. Use your assignment solution from Week 3 as a starting point for this week's assignment. Refer to the [Week 3 Assignment](/week3) for instructions.
...
- Create a new folder called `week7` inside your `app` folder.
- Copy `new-item.js`, `item-list.js`, `item.js`, `items.json`, and `page.js` from /week6 to /week7.
...
- Next, define a function called `fetchMealIdeas`, which fetch data from the API. `fetchMealIdeas` should take an ingredient as a parameter, make a fetch request to the [TheMealDB API](https://www.themealdb.com/), and return the meals that include that ingredient.
...
- Use the `useEffect` hook to call `fetchMealIdeas` whenever the `ingredient` prop changes. Store the result in the `meals` state variable.
