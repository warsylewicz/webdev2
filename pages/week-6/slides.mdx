...
- `splice()` - changes the contents of an array by removing elements
...
let removed = arr.splice(1, 2); // remove 2 elements starting at index 1, removed = [2, 3], arr = [1, 4, 5]
arr.reverse(); // arr = [5, 4, 1]
```
...
const index = names.findIndex((name) => name[2] === "v"); // find the index of the first name with a "v" at index 2, index = 3 (David)
```
