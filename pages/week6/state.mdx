...
function ParentComponent() {
  ...
  return (
    <div>
      <ComponentA count={count} incrementCount={incrementCount} />
      <ComponentB count={count} incrementCount={incrementCount} />
    </div>
  );
}

function ComponentA({ count, incrementCount }) {
  return (
    <div>
      <p>Count: {count}</p>
...
function ComponentB({ count, incrementCount }) {
  return (
    <div>
      <p>Count: {count}</p>
      <button onClick={incrementCount}>Increment</button>
...
In this example, the state of the counter (count and setCount) is stored in ParentComponent and passed down to ComponentA and ComponentB via props. When the button in either component is clicked, it calls the incrementCount function from ParentComponent, updating the state at the top level. This ensures both components always display the same count value.
...
- [Week 5 Demo code](https://github.com/warsylewicz/webdev2-demos/tree/master/app/week6)
