...
1. Use another student's Week 7 Assignment as a starting point for this week's assignment. Refer to the list of [student repositories](/github-accounts).

2. Use your assignment solution from Week 3 as a starting point for this week's assignment. Refer to the [Week 3 Assignment](/week3/assignment) for instructions.
...
Let's first create a new folder for this assignment and populating it with the files from previous assignments to use as a starting point. The main shopping list application is going to reside in `week8/shopping-list`. `week8` will be the landing page with login functionality. We are also going to add more structure to our application by creating a a `utils` folder to hold our Firebase code.

- Create a new folder called `week8` inside your `app` folder.
- Create a new folder called `shopping-list` inside your `week8` folder.
- Copy `page.js`, all components, and the `json` file from `week7` to `week8/shopping-list`.

We will add a `page.js` file to the `week8` folder later.

### Create Firebase Config

- Create a new folder called `_utils` inside your `week8` folder.
- Create a file called `firebase.js` inside your `week8/_utils` folder.
...
In the `week8` folder, create a file called `layout.js` and add the following code.
...
In the `week8` folder, create a `page.js` file. This will be the landing page for our application. Display a login button if the user is not logged in. If the user is logged in, display a welcome message, a logout button, and a link to the shopping list page.
...
In the `week8/shopping-list/page.js` file, check if the user is logged in by using the `useUserAuth` hook and if the `user` object is null, do not render the shopping list page. Optional: You can redirect the user to the landing page if you want.
...
https://cprg306-assignments.vercel.app/week8

Also, try to go to the shopping list page directly without signing in [https://cprg306-assignments.vercel.app/week8/shopping-list](https://cprg306-assignments.vercel.app/week8/shopping-list)
