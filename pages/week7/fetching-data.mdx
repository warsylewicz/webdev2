...
The Weather component has been updated to use live weather data for Calgary. As you read through the code for this component, pay attention to how the data is fetched from the API and how it's used to update the component's state.

Here are some things to look for:

1. **`useEffect` Hook:** The `useEffect` hook is used to perform side effects in the component. It takes two parameters: a function where you perform your side effects and a dependency array.
...
In this component, the `useEffect` hook is used to fetch data from a weather API. The function passed to `useEffect` is an Immediately Invoked Function Expression (IIFE) that is an async function. This is because `useEffect` cannot be an async function itself, but it can contain one.
...
2. **Fetching Data:** Inside the `useEffect` hook, a `fetch` request is made to a weather API. The URL includes parameters for latitude, longitude, and other weather details.
...
After the data is fetched, it is converted to a JavaScript object using `response.json()`. Then, the `setWeather` function is called to update the `weather` state with the fetched data.

3. **Rendering the Component:** In the return statement of the component, the fetched data is displayed. If the data hasn't been fetched yet (i.e., weather is still null), "unavailable" will be displayed.
...
The `?` used in `weather?.current.temperature_2m` and `weather?.current_units.temperature_2m` is optional chaining. It means that if `weather` or `weather.current` is `null` or `undefined`, it won't throw an error but will return `undefined`. This helps to avoid errors when trying to access properties of `null` or `undefined`.
