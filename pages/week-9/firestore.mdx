...
In Week 9 of the Community Events app, Cloud Firestore is used to store data for a simple event management application. The application allows users to view all the events, view one event, and add a new event. Please note that due to security rules, you will not be able to add an event to the database.
