...
- Next, define a function called `getMealIdeas`, which fetch data from the API. getMealIdeas should take an ingredient as a parameter, make a fetch request to the [TheMealDB API](https://www.themealdb.com/), and return the meals that include that ingredient.
...
- Use the `useEffect` hook to call getMealIdeas whenever the `ingredient` prop changes. Store the result in the `meals` state variable.
